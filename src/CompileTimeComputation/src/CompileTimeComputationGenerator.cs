...
public class CompileTimeComputationGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var syntaxProvider = context.SyntaxProvider
            .ForAttributeWithMetadataName(CompileTimeComputation, Select, Project)
            .Collect();
        var syntaxProviderWithCompilationProvider = syntaxProvider.Combine(
    ...
    private static FieldDeclaration Project(
        GeneratorAttributeSyntaxContext context,
        CancellationToken cancellationToken
    ) =>
        (
            Property: context.TargetNode is PropertyDeclarationSyntax
                ? context.TargetSymbol as IPropertySymbol
                : null,
            Method: context.TargetNode is PropertyDeclarationSyntax
                ? (context.TargetSymbol as IPropertySymbol).GetMethod
                : context.TargetNode is MethodDeclarationSyntax
                    ? context.TargetSymbol as IMethodSymbol
                    : null,
            Name: (
                context.Attributes
                    .FirstOrDefault(a => a.AttributeClass.Name == CompileTimeComputation)
            ...
            FieldType: (
                (
                    context.TargetNode is PropertyDeclarationSyntax
                        ? (context.TargetSymbol as IPropertySymbol).Type
                        : context.TargetNode is MethodDeclarationSyntax
                            ? (context.TargetSymbol as IMethodSymbol).ReturnType
                            : null
                ) as INamedTypeSymbol
            )!
        );
    ...
    public static void Execute(
        ...
        (ImmutableArray<FieldDeclaration> FieldDeclarations, Compilation Compilation) contexts
    )
    {
        var (fieldDeclarations, compilation) = contexts;
        context.AddSource(
            "ConstantFieldDeclarations.cs",
        ...
        context.ReportDiagnostic(
            Diagnostic.Create(
                new DiagnosticDescriptor(
                    "DEBUG01",
                    "",
                    $"DEBUG(compilation.GetDiagnostics().Length): *{compilation.GetDiagnostics().Length}*",
                    "DEBUG",
        ...
        context.ReportDiagnostic(
            Diagnostic.Create(
                new DiagnosticDescriptor(
                    "DEBUG02",
                    "",
                    $"DEBUG(fieldDeclarations.Length): *{fieldDeclarations.Length}*",
                    "DEBUG",
                    DiagnosticSeverity.Warning,
                    true
                ),
        ...
        foreach (var (property, method, name, fieldSymbol, fieldType) in fieldDeclarations)
        {
            ...
            context.ReportDiagnostic(
                Diagnostic.Create(
                    new DiagnosticDescriptor(
                        "DEBUG03",
                        $"DEBUG/fieldSymbolDisplay: {fieldSymbolDisplay}",
                        "",
                        "DEBUG",
                        DiagnosticSeverity.Warning,
                        true
