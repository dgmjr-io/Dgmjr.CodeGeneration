...
public class InjectedConstructorGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterPostInitializationOutput((i) => i.AddSource("InjectedAttribute.g.cs", SourceText.From(AttributeDeclaration, Encoding.UTF8)));
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        if (!(context.SyntaxReceiver is InjectedSyntaxReceiver receiver)) return;
        ...
        var properties = receiver.PropertiesWithInjectedAttribute
            .Select(s => compilation.GetSemanticModel(s.SyntaxTree).GetDeclaredSymbol(s) as IPropertySymbol)
            .Where(p => p != null);
        ...
        {
            code.AppendFormat("public partial class {0} {{ ", property.ContainingType.ToDisplayString());
            ...
            {
                if (attribute.AttributeClass.ToDisplayString() != "InjectedAttribute") continue;
            ...
            code.AppendFormat("public {0}({1}) {{ ", property.ContainingType.ToDisplayString(), string.Join(", ", parameterTypes.Zip(parameterNames, (t, n) => $"{t} {n}")));
            ...
            foreach (var ctor in property.ContainingType.Constructors)
            {
                ...
                var existingParameters = parameters.Any() ? ", " + string.Join(", ", parameters) : "";
                var overloadParameters = string.Join(", ", parameterTypes.Zip(parameterNames, (t, n) => $"{t} {n}"));

                code.AppendFormat("public {0}({1}{2}) : this({3}) {{ }} ", property.ContainingType.ToDisplayString(), string.Join(", ", parameters), existingParameters, overloadParameters);
            }
        ...
        context.AddSource("InjectedConstructorGenerator.cs", SourceText.From(code.ToString(), Encoding.UTF8));
    }
}
